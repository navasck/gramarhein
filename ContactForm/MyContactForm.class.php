...
class MyContactForm {
    ...
    function sendForm(){
        ...
        $mail2=    new PHPMailer();
	$mail2->IsHTML(true);
	$mail2->From = $this->from;
	$mail2->FromName  = $this->fromName;
	$mail2->Subject   = $this->subject;

	$mail2->Body      = $this->templateData;
        $toaddresses = $this->to;
	...
	if($this->bcc)
	 {
		$toaddresses = $this->bcc;
        foreach($toaddresses as $toAddress){
            $mail2->AddBCC($toAddress);
        }
	 }
	$send    =    $mail2->Send();
    }
    
    /** load the template into a variable for sending */
